...
let Bonsai = function (el, options) {
  let self = this;
  options = options || {};
...
Bonsai.prototype = {
  ...
  setExpanded(listItem, expanded) {
    ...
    if (expanded) {
      if (!$li.data('subList')) return null;
  ...
  update() {
    ...
    this.el.children().each(function () {
      ...
      let subLists = item.children().filter('ol, ul');
  ...
  serialize() {
    let idAttr = this.options.idAttribute;
    return this.el.find('li').toArray().reduce((acc, li) => {
      let $li = $(li);
      let id = $li.attr(idAttr);
      // only items with IDs can be serialized
      if (id) {
        let state = $li.hasClass('expanded')
            ? 'expanded'
  ...
  insertInput(listItem) {
    let type = this.options.createInputs;
    if (listItem.find(`> input[type=${type}]`).length) return;
    let id = this.inputIdFor(listItem);
    let checkbox = $(`<input type="${type}" name="${
       this.inputNameFor(listItem)}" id="${id}" /> `
    );
    let children = listItem.children();
    // get the first text node for the label
    let text = listItem.contents().filter(function () {
  ...
  handleDuplicateCheckboxes() {
    let self = this;
    self.el.on('change', 'input[type=checkbox]', (ev) => {
  ...
  addExpandAllLink() {
    ...
    $('<div class="expand-all">')
      .append(
        $('<a class="all">Expand all</a>').on('click', () => {
          self.expandAll();
        })
      )
      .append('<i class="separator"></i>')
      .append(
        $('<a class="none">Collapse all</a>').on('click', () => {
          self.collapseAll();
        })
      )
  ...
  setCheckedValues(values) {
    let all = this.options.scope.find('input[type=checkbox]');
...
$.fn.bonsai = function (options, ...args) {
  return this.each(function () {
    let bonsai = $(this).data('bonsai');
    if (!bonsai) {
